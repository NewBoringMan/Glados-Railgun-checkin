...
`status.py` 会在只读刷新时同时读取 `/api/user/points` 的积分历史，生成最近 35 天签到日历，并返回：

- `checked`：已签到；
- `missed`：在可确认的历史覆盖范围内没有签到记录；
- `pending`：今天尚未出现签到记录；
- `unknown`：GLaDOS 返回的历史没有覆盖到该日期，不能可靠判断，因此不会误标为漏签；
- `streak`：连续签到天数；如果状态接口本身提供 streak，则优先使用官方值，否则从积分历史推算。

Account Center V2.0.3 起会在账号卡片和独立“签到日历”页面显示这些状态。积分历史只有 `time + change` 而没有事件描述时，会把正积分变动作为兼容性签到信号；明确标记为兑换、邀请等奖励的记录不会被算作签到。
